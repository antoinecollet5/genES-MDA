Estimation of contaminant release history in groundwater based on concentration measurements

Problem Description
-------------------
This test case demonstrates the use of genES-MDA to estimate the time-dependent release history of a contaminant in an aquifer, based on observed concentration data sampled at sparse monitoring points over time.
The groundwater flow is assumed known and steady-state, precomputed using MODFLOW 2005. The contaminant transport (forward model) is modeled using MT3DMS.


Dependencies
------------
All required packages and their versions are listed in requirements.txt.

To install the required packages, run:
pip install -r requirements.txt

**Note:** No installation is required for MT3DMS as the executable file (`mt3d-usgs_1.1.0_64.exe`) is already provided within the `Model` subfolder of this example.


Execution
---------
To execute the application example, the Python command ES_MDA should be run using the default settings.


**Note:** The folder Tests_files contains various input files for different configurations, such as ensemble size, ensemble type, and observation errors.