...
def Func_ens(par, ens):
    ...
    (MinT,MaxT)=(1e-3,1e-5) # Define min and max bounds for the transmissivity (T) in m²/s
    (MinS,MaxS)=(1e-5,1e-6) # Define min and max bounds for the storativity (S) (dimensionless)
...
def Func_err(Obs, ens):
    ...
    N_obs=Obs.shape[0]
    var_y=1e-6     # Assumed observation error variance
...
def Metrics_obs_par(Xprev,pred,True_par,obs):
    ...
    metrics_dict ={}
    for variable in ['RMSE_obs', 'RMSE_par','rank_Qxy','rank_Qyy','rank_Qxx']:
        metrics_dict[variable]=eval(variable)
    return metrics_dict